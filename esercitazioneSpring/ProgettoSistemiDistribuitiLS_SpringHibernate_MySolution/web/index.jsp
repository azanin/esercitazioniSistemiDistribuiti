...
String printTableRow(Book book, String url) {
	...
	if ( book.getAuthors() != null ) {
		...
		for ( int i = 0; i < 3 && iterator.hasNext() ; i++ ) {
			authors[i] = ( iterator.next()).getName();
		}
	}
	for ( int i = 0; i < 3 ; i++ ) {
		html
...
String printTableRows(List<Book> books, String url) {
	StringBuffer html = new StringBuffer();
	Iterator<Book> iterator = books.iterator();
	while ( iterator.hasNext() ) {
		html.append( printTableRow( iterator.next(), url ) );
	}
	return html.toString();
}
%>
...
<%	
	...
	// either part of Spring (Spring MVC) or available as external library (Struts, JSF). This helper 
	// class is just the most generic way to access the root context. 
	
	// TODO: Application Context must be retrieved! How?
	ApplicationContext appContext = null; 
	
	AuthorDAO authorDAO = (AuthorDAO) appContext.getBean("authorDAO");
	BookDAO bookDAO = (BookDAO) appContext.getBean("bookDAO");
	PublisherDAO publisherDAO = (PublisherDAO) appContext.getBean("publisherDAO");
